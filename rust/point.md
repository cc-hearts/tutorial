...
Deref frait: 允许将Box引用的值当作引用来处理

Drop trait: 离开作用域或 heap 和 stack上的值都会被处理。
...
有时，一个值会有多个所有者
例如：为了支持多重所有权：Rc<T>（reference couting（引用计数））
追踪所有到值的引用:

...
用于保存 hooks 中的数据持久化 如果不调用 `setCount` 方法 则不会改变 count 的引用值
...
> 如果 Reducer Hook 的返回值与当前 state 相同(使用了 Object.is 比较两个值是否相等)，React 将跳过子组件的渲染及副作用的执行。
...
说明是一个函数 这个函数会将第二个参数作为形参穿入 函数的返回值会是作为 state 的初始值
